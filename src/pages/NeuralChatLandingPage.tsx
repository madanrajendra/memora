...
export default function NeuralChatLandingPage() {
          ...
          <div className="flex flex-col md:flex-row h-full min-h-[700px]">
            ...
            <aside className="w-full md:w-72 border-r border-white/5 p-6 flex flex-col gap-8 bg-black/20">
              ...
              <nav className="flex flex-row md:flex-col overflow-x-auto md:overflow-x-visible gap-2 md:gap-1 pb-2 md:pb-0 scrollbar-hide">
                {navigationItems.map((item) => {
                  const isActive = item.label === 'Neural Chat';
                  return (
                    <Link
                      key={item.label}
                      to={item.route}
            ...
            <main className="flex-1 flex flex-col relative bg-[#050505]">
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8">
                <div className="flex flex-col items-center text-center max-w-lg mb-20 animate-fade-in">
                  <img 
                    src="/assets/logo.png" 
                    alt="Memora" 
                    className="h-28 w-28 object-contain opacity-10 brightness-200 mb-8" 
                  />
                  <h2 className="font-orienta text-2xl uppercase tracking-[0.05em] leading-tight">
                    <span className="text-[#49a9ee]">Memora</span>{' '}
                    <span className="text-white">the core layer of</span>
                    <br />
                    <span className="text-white">future intelligence</span>
                  </h2>
                </div>
                      ...
                      <div className="rounded-[19px] bg-[#050505] p-4 flex flex-col gap-4">
                        <div className="flex flex-wrap gap-2">
                          {['Chat', 'Launch Workflow', 'Data Analysis'].map((mode) => (
                            <button
                              key={mode}
                              className="px-4 py-1.5 rounded-lg border border-white/5 bg-white/[0.03] text-[11px] font-medium text-white/60 hover:text-white hover:bg-white/10 hover:border-white/10 transition-all"
                            >
                              {mode}
                            </button>
                          ))}
