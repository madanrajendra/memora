...
export default function DecisionLabLandingPage() {
      ...
      <section className="px-8 pb-32">
        <div className="max-w-[1400px] mx-auto rounded-[32px] border border-white/10 bg-[#050507]/80 backdrop-blur-2xl shadow-[0_40px_100px_rgba(0,0,0,0.8)] overflow-hidden">
          <div className="flex flex-col md:flex-row h-full min-h-[750px]">
            ...
            <aside className="w-full md:w-72 border-r border-white/5 p-6 flex flex-col gap-8 bg-black/20">
              ...
              <nav className="flex flex-row md:flex-col overflow-x-auto md:overflow-x-visible gap-2 md:gap-1 pb-2 md:pb-0 scrollbar-hide">
                {navigationItems.map((item) => {
                  const isActive = item.label === 'Decision Assistant';
                  return (
                    <Link
                      key={item.label}
                      to={item.route}
            ...
            {/* Main Content Area - Decision Lab */}
              ...
              <div className="flex-1 flex flex-col p-8 overflow-y-auto custom-scrollbar">
                <div className="mb-12 animate-fade-in">
                  ...
                  <p className="text-[#8c8c94] text-lg max-w-2xl leading-relaxed">
                    Review branches, trade-offs, and high-signal outcomes across your cognitive history.
                  </p>
                </div>
                  ...
                  <div className="p-8 rounded-[24px] border border-white/5 bg-white/[0.02] backdrop-blur-sm group hover:border-white/10 transition-all">
                    <div className="flex items-center justify-between mb-8">
                       <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-[#742aff]/10 flex items-center justify-center">
                          <Binary className="h-6 w-6 text-[#742aff]" />
                        </div>
                        <div>
                          <h3 className="text-xl font-bold text-white">Signal Analysis</h3>
                          <p className="text-sm text-white/40">Probability & Variance</p>
                        </div>
                      </div>
                      <ArrowUpRight className="h-5 w-5 text-white/20 group-hover:text-[#742aff] transition-colors" />
                    </div>
                    
                    <div className="flex items-end gap-2 mb-6">
                      <div className="text-4xl font-bold text-white tracking-tighter">0.942</div>
                      <div className="text-sm text-[#742aff] font-bold mb-1">SIGNAL</div>
                    </div>
                    ...
                    <div className="h-[120px] flex items-end gap-1 px-2">
                       {[40, 70, 45, 90, 65, 80, 50, 85, 60, 95, 75, 55].map((h, i) => (
                         <div 
                           key={i}
                           className="flex-1 bg-white/5 rounded-t-sm group-hover:bg-[#742aff]/20 transition-all duration-500"
                  ...
                  <div className="p-8 rounded-[24px] border border-white/5 bg-white/[0.02] backdrop-blur-sm group hover:border-white/10 transition-all">
                    <div className="flex items-center gap-4 mb-8">
                      <div className="w-12 h-12 rounded-xl bg-[#0051FF]/10 flex items-center justify-center">
                        <Split className="h-6 w-6 text-[#0051FF]" />
                      </div>
                      <div>
                        <h3 className="text-xl font-bold text-white">Active Branches</h3>
                        <p className="text-sm text-white/40">Trajectory & Convergence</p>
                      </div>
                    </div>

                    <div className="space-y-4">
                       {[
                         { name: 'Core Strategy B', status: 'Converging', prob: '88%' },
                         { name: 'Identity Layer Sync', status: 'Stable', prob: '96%' },
                         { name: 'Resource Allocation', status: 'Diverging', prob: '42%' },
                       ].map((branch) => (
                         <div key={branch.name} className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.02] border border-white/5">
                            <div className="flex items-center gap-3">
                               <Activity className="h-4 w-4 text-white/40" />
                               <span className="text-sm font-medium text-white/80">{branch.name}</span>
                            </div>
                            <div className="flex items-center gap-4">
                               <span className="text-[10px] font-bold text-white/20 uppercase tracking-widest">{branch.status}</span>
                               <span className="text-sm font-mono text-[#0051FF]">{branch.prob}</span>
                            </div>
                         </div>
                       ))}
                    </div>
                   ...
                   <div className="relative group rounded-[20px] p-[1.5px] bg-gradient-to-r from-white/10 via-white/20 to-[#49a9ee]/40 shadow-[0_0_40px_rgba(54,163,229,0.1)] focus-within:shadow-[0_0_60px_rgba(73,169,238,0.2)] transition-all">
                      <div className="rounded-[19px] bg-[#050505] p-4 flex flex-col gap-4">
                        <div className="flex flex-wrap gap-2">
                          {['Simulate Branch', 'Risk Assessment', 'Analyze Outcome'].map((mode) => (
                            <button
                              key={mode}
                        ...
                        <div className="flex items-center justify-between gap-4 mt-2">
                          <input 
                            ...
                            placeholder="Input variables for decision simulation..."
                          />
